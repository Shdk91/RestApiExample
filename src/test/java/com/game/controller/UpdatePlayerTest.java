...
public class UpdatePlayerTest extends AbstractTest {
  ...
  public void updatePlayerWithIdTest() throws Exception {
    PlayerInfoTest expected = mapper
        .readValue(String.format(TestsHelper.NORMAL_JSON_WITH_ID, 5), PlayerInfoTest.class);
    expected.level = 35;
    ...
    assertNotEquals("При запросе POST /rest/players/{id} поле id не должно обновляться.", 8,
        actual.id);
    assertEquals(
        "При запросе POST /rest/players/{id} с id в теле запроса, должны быть обновлены поля, кроме поля id",
        expected, actual);
  }
  ...
  public void updatePlayerEmptyBodyTest() throws Exception {
    ...
    assertEquals(
        "При запросе POST /rest/players/{id} с пустым телом запроса, корабль не должен изменяться",
        expected, actual);
  }
  ...
  public void updatePlayerLevelTest() throws Exception {
    PlayerInfoTest expected = mapper
        .readValue(String.format(TestsHelper.NORMAL_JSON_WITH_ID, 23), PlayerInfoTest.class);
    expected.level = 35;
    expected.untilNextLevel = 2614;
    ...
    assertEquals(
        "При запросе POST /rest/players/{id} с level в теле запроса, должны быть обновлены поля, кроме поля level",
        expected, actual);
  }
  ...
  public void updatePlayerWithDataTest1() throws Exception {
    ...
    PlayerInfoTest expected = new PlayerInfoTest(playerInfoTest.id, newName, playerInfoTest.title,
        playerInfoTest.race, playerInfoTest.profession,
        playerInfoTest.birthday, newBanned, newExperience, 6, 300);

    ResultActions resultActions = mockMvc
        .perform(MockMvcRequestBuilders.post("/rest/players/" + playerInfoTest.id)
            .contentType(MediaType.APPLICATION_JSON)
    ...
    assertEquals(
        "При запросе POST /rest/players/{id} игрок должен обновляться, а рейтинг и остаток до следующего уровня пересчитываться",
        expected, actual);
  }
  ...
  public void updatePlayerWithDataTest2() throws Exception {
    ...
    PlayerInfoTest expected = new PlayerInfoTest(playerInfoTest.id, playerInfoTest.name, newTitle,
        newRace, newProfession, newBirthday,
        playerInfoTest.banned, playerInfoTest.experience, playerInfoTest.level,
        playerInfoTest.untilNextLevel);

    ResultActions resultActions = mockMvc
        .perform(MockMvcRequestBuilders.post("/rest/players/" + playerInfoTest.id)
            .contentType(MediaType.APPLICATION_JSON)
            .content(String.format(TestsHelper.JSON_SKELETON_2, newTitle, newRace, newProfession,
                newBirthday)))
        .andExpect(status().isOk());
    ...
    assertEquals(
        "При запросе POST /rest/players/{id} корабль должен обновляться и рейтинг пересчитываться",
        expected, actual);
  }
}